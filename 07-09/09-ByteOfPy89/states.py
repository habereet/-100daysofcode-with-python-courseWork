...
def get_state_abbrev(state_name, us_state_abbrev=us_state_abbrev):
	...
	return NOT_FOUND if state_name not in us_state_abbrev.keys() else us_state_abbrev[state_name]
...
def get_longest_state(data):
	"""Receives data, which can be the us_state_abbrev dict or the states
	   ...
	   of the string"""
	return max(data.keys(), key=len) if isinstance(data, dict) else max(data, key=len)
...
def combine_state_names_and_abbreviations(us_state_abbrev=us_state_abbrev, states=states):
	"""Get the first 10 state abbreviations ('AL', 'AK', 'AZ', ...) from
	   ...
	   alphabetical order"""
	return sorted(sorted(list(us_state_abbrev.values()))[0:10] + sorted(states)[40:])
